    ...
    try:
        with open(_marker_path, "r", encoding="utf-8") as _mf:
            if _mf.read().strip() == _rdna:
                print(
                    f"[setup_rocm] Already configured for {generation_name}. Nothing to do."
                )
                sys.exit(0)
    except OSError:
        pass
...
print("This script will install ROCm PyTorch + SDK wheels and patch bitsandbytes.\n")
...
if os.path.isfile(_pyproject_variant) and os.path.isfile(_lock_variant):
    ...
    print(
        "This may take several minutes on first run (subsequent runs use the local cache).\n"
    )
    ...
    _torch_url = (
        f"{_base}/torch-{_TORCH_VERSION}%2B{_ROCM_TAG}-cp{_py}-cp{_py}-win_amd64.whl"
    )
