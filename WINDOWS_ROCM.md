...
**`No solution found when resolving dependencies`** — You ran `uv run heretic` without `--frozen` after ROCm setup. The ROCm lock is Windows-only and cannot resolve on all platforms. Use `uv run heretic <model>` instead.
